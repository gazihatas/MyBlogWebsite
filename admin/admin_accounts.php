...
<section class="accounts">
   ...
   <h1 class="heading">Yönetici Hesapları</h1>

   <div class="box-container">

   <div class="box" style="order: -2;">
      <p>register new admin</p>
      <a href="register_admin.php" class="option-btn" style="margin-bottom: .5rem;">Kayıt Ol</a>
   </div>
   ...
   <div class="box" style="order: <?php if($fetch_accounts['id'] == $admin_id){ echo '-1'; } ?>;">
      <p> Yönetici id : <span><?= $fetch_accounts['id']; ?></span> </p>
      <p> Kullanıcı Adı : <span><?= $fetch_accounts['name']; ?></span> </p>
      <p> Toplam Gönderileri : <span><?= $total_admin_posts; ?></span> </p>
      <div class="flex-btn">
         <?php
            if($fetch_accounts['id'] == $admin_id){
         ?>
            <a href="update_profile.php" class="option-btn" style="margin-bottom: .5rem;">Güncelle</a>
            <form action="" method="POST">
               <input type="hidden" name="post_id" value="<?= $fetch_accounts['id']; ?>" on>
               <button type="submit" name="delete"onclick="return confirm('Silmek istediğinden emin misin?');" class="delete-btn" style="margin-bottom: .5rem;">Sil</button>
            </form>
         <?php
            }
         ?>
   ...
   <?php
      ...
      echo '<p class="empty">hesap yok!</p>';
   }
   ?>
